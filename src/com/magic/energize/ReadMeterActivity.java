...
public class ReadMeterActivity extends Activity {
	...
	public void submitReading(View view) {
		...
		if(meterUnits.equals(IMPERIAL))
			...
			title += "cubic meters.";
		String text = "Total Gigajouls Consumed: " + GJforReading();
		...
		alertBuilder.setMessage(text).setTitle(title)
	       ...
	       .setPositiveButton("SUBMIT", new DialogInterface.OnClickListener() {
	           public void onClick(DialogInterface dialog, int id) {
	        		...
	        		db.addMeterReading(convertedReading(), new Date().getTime());
	...
	private double GJforReading() {
		double reading = (double)convertedReading();
	...
	private int getReading() {
		...
		for(int i=0; i<readingValues.length; i++) {
			reading = Integer.parseInt(reading + "" + readingValues[i]);
		}
		if(meterUnits.equals(IMPERIAL)) {
